...
import {AcceptTeamMember} from "../../../modules/teams/state/team.action";
import {HandleError, SetNotifications} from "../../../shared/state/global.action";
import {TeamStateModel} from "../../../modules/teams/state/team.state";
...
export class NotificationsState {
  ...
  constructor(private nzNotificationService: NzNotificationService, private notificationsService: NotificationsService, private globalService: GlobalService,
              private store: Store) {
  }
  ...
  acceptNotificationRequest({getState, setState}: StateContext<NotificationsStateModel>, payload: any) {
    const state = getState()
    return this.globalService.updateRequest(payload.id , payload.requiredInputs , payload.requiredAccounts).pipe(tap((res) => {
      this.store.dispatch(new SetNotifications('Success', res?.message, 'success'))
  ...
  addSocketNotification({getState, setState}: StateContext<NotificationsStateModel>, payload: any) {
    ...
    if(state.notifications.data.length > 0){
      let arrayNotifications = state.notifications.data
